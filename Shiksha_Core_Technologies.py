...
# ================= NOTES FUNCTIONS - SUBJECT-WISE =================
def save_note(note_text, subject):
    """Save a note with subject categorization"""
    ...
    return f"✅ Note saved at {timestamp} under {subject}"


def view_notes(filter_subject="All Subjects"):
    """View all saved notes or filter by subject"""
    try:
        with open(NOTES_FILE, 'r') as f:
            content = f.read()
        ...
        # If viewing all subjects, return everything
        if filter_subject == "All Subjects":
            return f"# 📝 My Notes - All Subjects\n\n{content}"
        ...
        for note in notes:
            if note.strip() and f"Subject: {filter_subject}" in note:
                filtered_notes.append(note)

        if not filtered_notes:
            return f"# 📝 My Notes - {filter_subject}\n\n_No notes found for {filter_subject}_"
...
# JavaScript to suppress error popups
SUPPRESS_ERRORS_JS = """
<script>
(function() {
    // Suppress console errors related to component updates
    const originalError = console.error;
    console.error = function(...args) {
        const errorStr = args.join(' ');
        if (errorStr.includes('Component') || 
            errorStr.includes('update') || 
    ...
    style.textContent = `
        .toast-wrap, .toast, [class*="toast"], [class*="Toast"] {
            display: none !important;
            visibility: hidden !important;
            opacity: 0 !important;
            pointer-events: none !important;
    ...
    // Monitor and remove toast elements
...
with gr.Blocks(title="📚 EXAM_EPS - Smart Study Platform",
    ...
    with gr.Column(visible=False) as dashboard_page:
        ...
        with gr.Tabs():
            ...
            with gr.TabItem("📝 Notes"):
                ...
                # Subject buttons for saving notes
                gr.Markdown("### 📚 Select Subject")
                note_subject = gr.State("Mathematics")

                with gr.Row():
                    math_btn = gr.Button("📐 Mathematics", elem_classes="activity-btn")
                    physics_btn = gr.Button("⚛️ Physics", elem_classes="activity-btn")
                    chemistry_btn = gr.Button("🧪 Chemistry", elem_classes="activity-btn")
                    biology_btn = gr.Button("🧬 Biology", elem_classes="activity-btn")

                with gr.Row():
                    english_btn = gr.Button("📖 English", elem_classes="activity-btn")
                    sst_btn = gr.Button("🌍 SST", elem_classes="activity-btn")
                    hindi_btn = gr.Button("🔤 Hindi", elem_classes="activity-btn")
                    ai_btn = gr.Button("🤖 AI", elem_classes="activity-btn")
                    it_btn = gr.Button("💻 IT", elem_classes="activity-btn")

                selected_subject_display = gr.Markdown("**Selected Subject:** Mathematics")
                ...
                # Subject buttons for viewing notes
                gr.Markdown("### 🔍 View Notes by Subject")
                view_subject_filter = gr.State("All Subjects")

                with gr.Row():
                    view_all_btn = gr.Button("📚 All Subjects", elem_classes="primary-btn")
                    view_math_btn = gr.Button("📐 Mathematics", elem_classes="activity-btn")
                    view_physics_btn = gr.Button("⚛️ Physics", elem_classes="activity-btn")
                    view_chemistry_btn = gr.Button("🧪 Chemistry", elem_classes="activity-btn")

                with gr.Row():
                    view_biology_btn = gr.Button("🧬 Biology", elem_classes="activity-btn")
                    view_english_btn = gr.Button("📖 English", elem_classes="activity-btn")
                    view_sst_btn = gr.Button("🌍 SST", elem_classes="activity-btn")
                    view_hindi_btn = gr.Button("🔤 Hindi", elem_classes="activity-btn")

                with gr.Row():
                    view_ai_btn = gr.Button("🤖 AI", elem_classes="activity-btn")
                    view_it_btn = gr.Button("💻 IT", elem_classes="activity-btn")
    ...
    # Notes - Modified to include subject
    # Subject selection button handlers
    def select_subject(subject_name):
        return subject_name, f"**Selected Subject:** {subject_name}"

    math_btn.click(lambda: select_subject("Mathematics"), outputs=[note_subject, selected_subject_display])
    physics_btn.click(lambda: select_subject("Physics"), outputs=[note_subject, selected_subject_display])
    chemistry_btn.click(lambda: select_subject("Chemistry"), outputs=[note_subject, selected_subject_display])
    biology_btn.click(lambda: select_subject("Biology"), outputs=[note_subject, selected_subject_display])
    english_btn.click(lambda: select_subject("English"), outputs=[note_subject, selected_subject_display])
    sst_btn.click(lambda: select_subject("SST"), outputs=[note_subject, selected_subject_display])
    hindi_btn.click(lambda: select_subject("Hindi"), outputs=[note_subject, selected_subject_display])
    ai_btn.click(lambda: select_subject("AI"), outputs=[note_subject, selected_subject_display])
    it_btn.click(lambda: select_subject("IT"), outputs=[note_subject, selected_subject_display])

    def save_note_and_refresh(note_text, subject):
        """Save note with subject and refresh stats"""
        msg = save_note(note_text, subject)
        _, _, _, notes_html = get_stats_html()
        return msg, notes_html

    save_note_btn.click(save_note_and_refresh,
                        inputs=[note_input, note_subject],
                        outputs=[notes_output,
                                 stat_notes]).then(lambda: "",
                                                   outputs=note_input)

    # View notes button handlers
    def view_and_display(subject):
        return subject, view_notes(subject)

    view_all_btn.click(lambda: view_and_display("All Subjects"), outputs=[view_subject_filter, notes_output])
    view_math_btn.click(lambda: view_and_display("Mathematics"), outputs=[view_subject_filter, notes_output])
    view_physics_btn.click(lambda: view_and_display("Physics"), outputs=[view_subject_filter, notes_output])
    view_chemistry_btn.click(lambda: view_and_display("Chemistry"), outputs=[view_subject_filter, notes_output])
    view_biology_btn.click(lambda: view_and_display("Biology"), outputs=[view_subject_filter, notes_output])
    view_english_btn.click(lambda: view_and_display("English"), outputs=[view_subject_filter, notes_output])
    view_sst_btn.click(lambda: view_and_display("SST"), outputs=[view_subject_filter, notes_output])
    view_hindi_btn.click(lambda: view_and_display("Hindi"), outputs=[view_subject_filter, notes_output])
    view_ai_btn.click(lambda: view_and_display("AI"), outputs=[view_subject_filter, notes_output])
    view_it_btn.click(lambda: view_and_display("IT"), outputs=[view_subject_filter, notes_output])
    ...
    timer_update.tick(get_timer_html, outputs=timer_display, show_progress=False)

    # Stats refresh timer (every 5 seconds)
    stats_update = gr.Timer(5)
    stats_update.tick(get_stats_html, outputs=[stat_study_time, stat_goals, stat_tasks, stat_notes], show_progress=False)
